...
The `home-routes` handler in the `ejemplo-re-frame.routes.home` namespace
...
The `home-page` function will in turn call the `ejemplo-re-frame.layout/render` function
...
The rest of this page is rendered by ClojureScript found in the `src/cljs/ejemplo_re_frame/core.cljs` file.



#### Organizing the routes

The routes are aggregated and wrapped with middleware in the `ejemplo-re-frame.handler` namespace:
...
Request middleware functions are located under the `ejemplo-re-frame.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `ejemplo-re-frame.dev-middleware` namespace found in
